...
(async () => {
  // Use the preinstalled Chromium if the npm Playwright version expects a different browser build.
  let browser;
  try { browser = await chromium.launch(); }
  catch (e) {
    const fs2 = require('fs');
    const cands = [process.env.CHROMIUM_PATH, '/opt/pw-browsers/chromium'].filter(Boolean);
    const exe = cands.find(c => { try { return fs2.statSync(c).isFile(); } catch (_) { return false; } })
      || (() => { try { const d = fs2.readdirSync('/opt/pw-browsers').filter(x => /^chromium-\d+$/.test(x)).sort().pop();
            return d && '/opt/pw-browsers/' + d + '/chrome-linux/chrome'; } catch (_) { return null; } })();
    if (!exe) throw e;
